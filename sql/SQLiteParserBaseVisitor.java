...
public class SQLiteParserBaseVisitor<T> extends AbstractParseTreeVisitor<T> implements SQLiteParserVisitor<T> {
	...
	Set<String> created_columns = new HashSet<>(); // todo
	List<Integer> numbers_stack = new LinkedList();
	...
	@Override public T visitSql_stmt_list(SQLiteParser.Sql_stmt_listContext ctx) {
	    // System.out.println("Visiting SQL statement list");
	
	    // Loop through each sql_stmt in the list
	    for (SQLiteParser.Sql_stmtContext sqlStmt : ctx.sql_stmt()) {
	        // System.out.println("Visiting SQL statement: " + sqlStmt.getText());
	        visit(sqlStmt);  // Process each individual SQL statement
	    }
	...
	@Override public T visitCreate_table_stmt(SQLiteParser.Create_table_stmtContext ctx) { 
		...
		// todo: done
		// System.out.println("hi elise");

		List<SQLiteParser.Column_defContext> columns = ctx.column_def();
		for (SQLiteParser.Column_defContext currcol : columns) {
		    // System.out.println("Adding column: " + currcol.getText());
			created_columns.add(currcol.getText());
		}
	 ...
	 * {@link #visitChildren} on {@code ctx}.</p>
	 */
	@Override public T visitCreate_virtual_table_stmt(SQLiteParser.Create_virtual_table_stmtContext ctx) { 
	
		// todo!

		// System.out.println("elise string: " + ctx.getText());
		// System.out.println("get test ver: " + ctx.module_argument().get(0).getText());

		List<SQLiteParser.Module_argumentContext> columns = ctx.module_argument();
		for (SQLiteParser.Module_argumentContext currcol : columns) {
		    // System.out.println("Adding column: " + currcol.getText());
			created_columns.add(currcol.getText());
		}
	 ...
	 * {@link #visitChildren} on {@code ctx}.</p>
	 */
	@Override public T visitExpr(SQLiteParser.ExprContext ctx) { 
		
		// System.out.println("elise hi hows it"); // todo
		// System.out.println("statement in expr: " + ctx.getText());

		if(ctx.PLUS() != null){
			// System.out.println("lit: " + ctx.PLUS().getText());
			numbers_stack.add(2);
		}else if(ctx.literal_value() != null && ctx.literal_value().NUMERIC_LITERAL() != null){
			// System.out.println("lit: " + ctx.literal_value().NUMERIC_LITERAL().getText());
			int popped = numbers_stack.remove(numbers_stack.size() - 1);
			if(popped != 0){
				numbers_stack.add(popped);
			}
			try{
				int x = Integer.parseInt(ctx.literal_value().NUMERIC_LITERAL().getText());
			}catch(NumberFormatException e){
				// System.out.println("Not an Integer");
				throw new RuntimeException("Not an Integer");
			}
		} else if((ctx.literal_value() != null) && !numbers_stack.isEmpty() && (ctx.literal_value().STRING_LITERAL() != null)){
			// System.out.println("string in plus!!!");
			throw new RuntimeException("Can only add integers");
		}
	...
	@Override public T visitSelect_stmt(SQLiteParser.Select_stmtContext ctx) { 
		...
		SQLiteParser.Select_coreContext columns = ctx.select_core().get(0);
		
		for (int i = 0; i < columns.Select_coreContext().size(); i++) {

		    // System.out.println("Column: " + columns.result_column(i).getText() + "\nsize:" + columns.Select_coreContext().size());
			
			// if((columns.result_column(i).column_alias() != null) && !created_columns.contains(columns.result_column(i).column_alias().getText())){
			if((columns.result_column(i).expr().column_name() != null) && !created_columns.contains(columns.result_column(i).expr().column_name().getText())){

				// System.out.println("broken!!!! - columns one");
				// System.out.println("inside: " + columns.result_column(i).column_alias().getText());
				// System.out.println("result column: " + columns.result_column(i).expr().column_name().getText());

				throw new RuntimeException("Columns not created beforehand");
			}
		}
	 ...
	 * {@link #visitChildren} on {@code ctx}.</p>
	 */
	@Override public T visitSimple_function_invocation(SQLiteParser.Simple_function_invocationContext ctx) { 
		
		// does not go here
		// System.out.println("elise johnson");
